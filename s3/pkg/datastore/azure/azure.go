...
func (ad *AzureAdapter) CompleteMultipartUpload(ctx context.Context, multipartUpload *pb.MultipartUpload,
	...
	pro, err := blobURL.GetProperties(ctx, azblob.BlobAccessConditions{})
	if err != nil {
		log.Errorf("get blob[objectId:%s] property failed:%v\n", multipartUpload.ObjectId, err)
		return nil, ErrBackendCompleteMultipartFailed
	}

	log.Debugf("pro: %+v\n", *pro)
	result.Size = pro.ContentLength()
	log.Infof("complete multipart upload[Azure Blob], bucket:%s, objectId:%s, size:%d succeed\n",
		bucket, multipartUpload.ObjectId, result.Size)
	return &result, nil
}
