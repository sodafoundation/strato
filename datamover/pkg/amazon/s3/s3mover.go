...
func (mover *S3Mover) UploadObj(objKey string, destLoca *LocationInfo, buf []byte) error {
	...
	if err != nil {
		log.Infof("[s3mover] New session failed, err:%v\n", err)
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] Upload object[%s] failed %d times, err:%v\n", objKey, tries, err)
...
func (mover *S3Mover) DownloadObj(objKey string, srcLoca *LocationInfo, buf []byte) (size int64, err error) {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover]download object[bucket:%s,key:%s] failed %d times, err:%v\n",
...
func (mover *S3Mover) MultiPartDownloadInit(srcLoca *LocationInfo) error {
	...
	if err != nil {
		log.Infof("[s3mover] new session for multipart download failed, err:%v\n", err)
		return handleAWSS3Errors(err)
	}
...
func (mover *S3Mover) DownloadRange(objKey string, srcLoca *LocationInfo, buf []byte, start int64, end int64) (size int64, err error) {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] download object[%s] range[%d - %d] failed %d times, err:%v\n",
...
func (mover *S3Mover) MultiPartUploadInit(objKey string, destLoca *LocationInfo) (string, error) {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] init multipart upload[objkey:%s] failed %d times, err:%v.\n", objKey, tries, err)
			e := handleAWSS3Errors(err)
			if tries >= 3 || e.Error() == DMERR_NoPermission { //If no permission, then no need to retry.
				return "", e
			}
...
func (mover *S3Mover) UploadPart(objKey string, destLoca *LocationInfo, upBytes int64, buf []byte, partNumber int64, offset int64) error {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] upload range[objkey:%s, partnumber#%d, offset#%d] failed %d times, err:%v\n",
...
func (mover *S3Mover) AbortMultipartUpload(objKey string, destLoca *LocationInfo) error {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] abort multipart upload[objkey:%s] for uploadId#%s failed %d times, err:%v.\n",
...
func (mover *S3Mover) CompleteMultipartUpload(objKey string, destLoca *LocationInfo) error {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] completeMultipartUpload [objkey:%s] failed %d times, err:%v\n", objKey, tries, err)
			e := handleAWSS3Errors(err)
			if tries >= 3 || e.Error() == DMERR_NoPermission { //If no permission, then no need to retry.
				return e
			}
...
func (mover *S3Mover) DeleteObj(objKey string, loca *LocationInfo) error {
	...
	for tries := 1; tries <= 3; tries++ {
		...
		if err != nil {
			log.Infof("[s3mover] delete object[key:%s] from bucket %s failed %d times, err:%v\n",
			...
			if err != nil {
				log.Infof("[s3mover] error occurred while waiting for object[%s] to be deleted.\n", objKey)
			} else {
...
func ListObjs(loca *LocationInfo, filt *pb.Filter) ([]*s3.Object, error) {
	...
	e := handleAWSS3Errors(err)
	if e != nil {
		log.Infof("[s3mover] list aws bucket failed, err:%v\n", e)
		return nil, e
	}
	...
	for *output.IsTruncated == true {
		...
		e := handleAWSS3Errors(err)
		if e != nil {
			log.Infof("[s3mover] list objects failed, err:%v\n", e)
			return nil, e
		}
