...
func RegisterRouter(ws *restful.WebService) {
	...
	ws.Route(ws.PUT("/{bucketName}").To(handler.RouteBucketCreate)).Doc("Create bucket for the user")
	ws.Route(ws.PUT("/{bucketName}/{folderName}").To(handler.RouteBucketPut)).Doc("Create folder under bucket for the user")
