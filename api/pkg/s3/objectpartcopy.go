...
func (s *APIService) ObjectPartCopy(request *restful.Request, response *restful.Response) {
	...
	sourceObject, err := s.s3Client.GetObjectMeta(ctx, &pb.Object{
		ObjectKey:  sourceObjectName,
		BucketName: sourceBucketName,
	})
	if HandleS3Error(response, request, err, sourceObject.ErrorCode) != nil {
		log.Errorf("unable to fetch object info. err:%v, errCode:%v", err, sourceObject.ErrorCode)
		return
	}

	// Verify before x-amz-copy-source preconditions before continuing with CopyObject.
	if err = checkObjectPreconditions(response.ResponseWriter, request.Request, sourceObject.Object); err != nil {
	...
	if copySourceRangeString == "" {
		readOffset = 0
		readLength = sourceObject.Object.Size
	} else {
		copySourceRange, err := datatype.ParseRequestRange(copySourceRangeString, sourceObject.Object.Size)
		if err != nil {
